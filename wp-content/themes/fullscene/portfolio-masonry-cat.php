<?php
/*
Template Name: Portfolio Page - Masonry - One Category
*/

$folioCatOption = get_post_meta($post->ID, 'folioCat', TRUE);
$folioCats = get_categories('taxonomy=portfolio_cats&child_of='.$folioCatOption);
        ...
        <li class="<?php if( !empty($folio_cats) ){ foreach( $folio_cats as $filter_class ){ echo $filter_class->slug.' '; } } ?>all folio-item block-bg">
          <a class="folio-overlay" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
            ...
            if ( has_post_thumbnail()):
              the_post_thumbnail('masonry-thumb', $imgAtt);
            else:?>
